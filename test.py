...
def run(fileTrain, fileTest, tohop):
    ...
    predict = dc_predict[0][0]

    res = "Điểm dự đoán: " + "{:.2f}".format(predict) + "\n"
    res += "Hệ số: " + str(reg_ridge.coef_) + "\n"

    print('Điểm chuẩn: ', y_test)
    print('Điểm dự đoán: ', predict)
    ...
    diem_du_doan = np.append(diem_du_doan, [predict])
